...
static void describe_monster_drop(int r_idx, const monster_lore *l_ptr)
{
	...
	if (l_ptr->drop_gold || l_ptr->drop_item)
	{
		...
		/* Chests are not noted as good or great (no "n" needed) */
...
static void describe_monster_toughness(int r_idx, const monster_lore *l_ptr)
{
	...
	{
		/* Armor */
		text_out(format("%^s has an armor rating of %d", wd_he[msex], r_ptr->ac));
...
static void describe_monster_exp(int r_idx, const monster_lore *l_ptr)
{
	...
	{
		/* Introduction */
		text_out( (l_ptr->r_l_flags1 & RF1_UNIQUE) ? "Killing" :"A kill of");
		...
		text_out(format(" is worth %ld.%02ld point%s", (long)i, (long)j,
			       (((i == 1) && (j == 0)) ? "" : "s")));
